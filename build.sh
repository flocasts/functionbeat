...
$AWS lambda list-functions | awk "/FunctionName/ && /${namespace}/ { print \$2 }" \
  | egrep -v 'logger|datadog|testola|thumb|jasons|matts' \
...
for fn in `cat log_groups.txt`; do
  ...
  entry="      - log_group_name: /aws/lambda/${fn}"
...
for conf in `ls configs`; do
  ./functionbeat deploy -c functionbeat.yml -c configs/${conf} -e -d "*" ${namespace}-log-processor${iter}
  iter=$((iter + 1))
done